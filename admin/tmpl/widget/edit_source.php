				...
				foreach ($this->corePlugins as $corePlugin)
				{
								...
								?><div class="col-12 col-md-3">
									<div class="source-tile p-4 text-center">
										...
										<button class="btn btn-success btn-source mt-3" data-source="<?php echo $corePlugin['type']; ?>">
											<span class="fas fa-check" aria-hidden="true"></span>&nbsp;&nbsp;<?php echo Text::_('COM_MINITEKSLIDER_WIDGET_SELECT'); ?>
										</button>
									</div>
								</div><?php
							}
								...
								?><div class="col-12 col-md-3">
									<div class="source-tile p-4 text-center">
										...
										<a href="<?php echo $corePlugin['downloadurl']; ?>" class="btn btn-info mt-3" target="_blank">
											<span class="fas fa-lock" aria-hidden="true"></span>&nbsp;&nbsp;<?php echo Text::_('COM_MINITEKSLIDER_DASHBOARD_UPGRADE_TO_PRO'); ?>
										</a>
									</div>
								</div><?php
							}
					...
					if (!$corePluginEnabled)
					{
						if ($corePlugin['type'] == 'content')
						{
							...
							if ($sourcePlugin = MinitekSliderHelper::getSourcePlugin($corePlugin['type']))
							{
								$title = Text::_('COM_MINITEKSLIDER_WIDGET_PUBLISH');
								$icon = 'check';
							...
							// Not installed
							{
								$title = Text::_('COM_MINITEKSLIDER_WIDGET_INSTALL');
								$icon = 'download';
