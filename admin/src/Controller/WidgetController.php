...
namespace Joomla\Component\MinitekSlider\Administrator\Controller;
...
class WidgetController extends FormController
{
	...
	public function cancelSource()
	{
		...
		// Redirect
		$app->redirect('index.php?option=com_minitekslider&view=widget&layout=edit&id='.$this_id);
	}
	...
	public function selectSource()
	{
		...
		$this_id = $app->input->get('id');
		$app->setUserState('com_minitekslider.source_id', $source_type);
		...
		{
			$app->redirect('index.php?option=com_minitekslider&view=widget&layout=edit');
		}
	}
	...
	public function createModule()
	{
		...
		if ($id)
		{
			...
			{
				// Redirect to module
				$app->enqueueMessage(\JText::_('COM_MINITEKSLIDER_WIDGET_MODULE_CREATED'), 'Message');
				$app->redirect('index.php?option=com_modules&filter.search=id:'.$data);
			}
			else
			{
				// Redirect to widgets
				$app->enqueueMessage(\JText::_('COM_MINITEKSLIDER_WIDGET_ERROR_WHILE_CREATING_MODULE'), 'Notice');
				$app->redirect('index.php?option=com_minitekslider&view=widgets');
			}
		}
		else
		{
