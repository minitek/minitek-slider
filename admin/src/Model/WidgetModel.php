...
namespace Joomla\Component\MinitekSlider\Administrator\Model;
...
class WidgetModel extends AdminModel
{
	 ...
	 * @since  4.0.0
	 */
	protected $text_prefix = 'COM_MINITEKSLIDER';
	 ...
	 * @since  4.0.0
	 */
	public $typeAlias = 'com_minitekslider.widget';
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return \JFactory::getUser()->authorise('core.delete', 'com_minitekslider');
		}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_minitekslider.widget.' . (int) $record->id);
		}
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$query = $db->getQuery(true);
			$query->select('*')
				->from('#__minitek_slider_widgets_source');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_minitekslider.widget', 'widget', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_minitekslider.widget.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_minitekslider')))
		{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_minitekslider.slider', 'slider', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = \JFactory::getApplication()->getUserState('com_minitekslider.edit.widget.data', array());
		...
		$this->preprocessData('com_minitekslider.widget', $data);
	...
	public function save($data)
	{
		...
		// Slider params
		$slider_params = $formData->get('slider_params', false);
		if ($slider_params && is_object($slider_params))
		{
			$registry = new Registry($slider_params);
		...
		if (parent::save($data))
		{
			...
			if (array_key_exists('source_type', $data['source_params']) && ($source_id == $data['source_params']['source_type']))
			{
				if (!$data['id'])
				{
					...
					$query
						->insert($db->quoteName('#__minitek_slider_widgets_source'))
						->columns($columns)
	...
	public function createModule($id, $position)
	{
		...
		$query = $db->getQuery(true);
		$query->select('*')
			->from('#__minitek_slider_widgets AS s');
		...
		$values = array($db->quote($widget_name), $db->quote(''), $db->quote($position), $db->quote('mod_minitekslider'), $db->quote('1'), $db->quote($widget_params), $db->quote('*'));
