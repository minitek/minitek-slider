...
class com_miniteksliderInstallerScript
{
	/*
	...
	function preflight($type, $parent)
	{
		...
		{
			throw new GenericDataException('Cannot install Minitek Slider version <strong>'.$this->new_version.'</strong> over <strong>Joomla 3</strong>. Please update to <strong>Joomla 4</strong> first.', 500);
	...
	private static function getInstalledVersion()
	{
		...
		$query .= ' WHERE ' . $db->quoteName('element').' = '.$db->quote('com_minitekslider');
		$db->setQuery($query);
	 ...
	 */
	private static function update406($parent)
	{
		/*
		...
		$query->select('*');
		$query->from($db->quoteName('#__minitek_slider_widgets'));
		$db->setQuery($query);
		...
		{
			throw new GenericDataException('Error 4.0.6: Could not read widgets.', 500);

			return false;
		}

		/* 
		 * Update __minitek_slider_widgets
		 */

		$widgets_columns = $db->getTableColumns('#__minitek_slider_widgets');
		...
		$query->update($db->quoteName('#__minitek_slider_widgets'))
			->set($fields)
			->where($conditions);
		$db->setQuery($query);
		$result = $db->execute();
		...
		/* 
		 * Update __minitek_slider_widgets_source
		 */

		$widgets_source_columns = $db->getTableColumns('#__minitek_slider_widgets_source');

		// Add column 'source_params' in #__minitek_slider_widgets_source
		if (!isset($widgets_source_columns['source_params']))
		{
			$query = $db->getQuery(true);
			$query = " ALTER TABLE `#__minitek_slider_widgets_source` ";
			$query .= " ADD COLUMN `source_params` text DEFAULT NULL ";
			$db->setQuery($query);

			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not add column source_params.', 500);
		...
		$query = $db->getQuery(true);
		$query->select('*');
		$query->from($db->quoteName('#__minitek_slider_widgets_source'));
		$db->setQuery($query);
		...
		{
			throw new GenericDataException('Error 4.0.6: Could not read widgets sources.', 500);
		...
		if ($sources)
		{
			foreach ($sources as $source)
			{
				...
				if ($source->joomla_source)
				{
					...
					$query = $db->getQuery(true);
					$query
						->update($db->quoteName('#__minitek_slider_widgets_source'))
		...
		if (isset($widgets_source_columns['joomla_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column joomla_source.', 500);
		...
		if (isset($widgets_source_columns['k2_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column k2_source.', 500);
		...
		if (isset($widgets_source_columns['virtuemart_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column virtuemart_source.', 500);
		...
		if (isset($widgets_source_columns['jomsocial_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column jomsocial_source.', 500);
		...
		if (isset($widgets_source_columns['easyblog_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column easyblog_source.', 500);
		...
		if (isset($widgets_source_columns['folder_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column folder_source.', 500);
		...
		if (isset($widgets_source_columns['rss_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column rss_source.', 500);
		...
		if (isset($widgets_source_columns['easysocial_source']))
		{
			...
			if (!$result = $db->execute())
			{
				throw new GenericDataException('Error 4.0.6: Could not delete column easysocial_source.', 500);
	...
	private static function installModule()
	{
		$db = Factory::getDBO();
 		$query = $db->getQuery(true)
 			->select('*')
 			->from($db->quoteName('#__extensions'))
 			->where($db->quoteName('element').' = '.$db->quote('mod_minitekslider'));
 		$db->setQuery($query);
		...
		if (!$result = $db->loadObject())
		{
			$app = Factory::getApplication();
			$input = $app->input;
			$xml = 'https://www.minitek.gr/index.php?option=com_ars&view=update&task=stream&format=xml&id=41';
